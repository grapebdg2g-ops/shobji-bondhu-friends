...
export function AiChatView({ sessionId: initialSessionId }: { sessionId?: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error || !data) {
        toast.error("চ্যাট খুঁজে পাওয়া যায়নি");
        return;
      }
  ...
  const sendText = async (text: string, opts: { skipCache?: boolean } = {}) => {
    ...
    try {
      ...
      setMessages(updated);
      suggest({
        data: { messages: updated.slice(-6).map(({ role, content }) => ({ role, content })) },
      })
  ...
  const submitFeedback = async (idx: number, helpful: boolean) => {
    ...
    setMessages((prev) =>
      prev.map((x, i) => (i === idx ? { ...x, feedback: helpful ? "up" : "down" } : x)),
    );
    try {
  ...
  const saveChat = async () => {
    ...
    if (!firstUser) {
      toast.error("সেভ করার মতো কিছু নেই");
      return;
    }
    setSaving(true);
    const newTitle =
      firstUser.content.slice(0, 40) + (firstUser.content.length > 40 ? "..." : "") || "নতুন চ্যাট";
    ...
    <aside className="h-full w-64 bg-card border-r border-border flex flex-col">
      ...
      <div className="p-3">
        <button
          ...
          className="w-full h-10 rounded-lg bg-primary text-primary-foreground text-sm font-semibold flex items-center justify-center gap-2 hover:bg-primary/90 active:scale-[0.98] transition"
        >
      ...
      <div className="px-3 pb-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-9 pl-8 pr-3 rounded-lg bg-muted text-sm outline-none focus:ring-2 focus:ring-primary/30"
          />
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        {sessions.length === 0 && (
          <p className="text-xs text-muted-foreground text-center mt-6 px-3">
            কোনো সেভ করা চ্যাট নেই। প্রথম প্রশ্ন করে সেভ করুন।
          </p>
        )}
        {(["আজ", "গতকাল", "এই সপ্তাহ", "পুরনো"] as const).map((label) =>
          grouped[label].length ? (
            <div key={label} className="mt-3">
              <div className="px-2 text-[10px] font-semibold text-muted-foreground tracking-wider uppercase mb-1">
                {label}
              </div>
              {grouped[label].map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className={`group relative rounded-lg mb-0.5 ${active ? "bg-primary/10 border-l-2 border-primary" : "hover:bg-muted"}`}
                  >
                    {renamingId === s.id ? (
                      <div className="flex items-center gap-1 px-2 py-1.5">
                        <input
                          ...
                          className="flex-1 min-w-0 h-7 px-2 text-xs rounded border border-border outline-none"
                        />
                        <button
                      ...
                      <button
                        ...
                        className="w-full text-left px-2 py-2 flex items-center gap-2 text-xs text-muted-foreground"
                      >
                        ...
                        <span
                          ...
                          className="h-6 w-6 rounded hover:bg-muted flex items-center justify-center opacity-0 group-hover:opacity-100 transition"
                        >
                    ...
                    {menuOpenFor === s.id && (
                      <div className="absolute right-2 top-9 z-10 bg-card border border-border rounded-lg shadow-lg py-1 w-40">
                        <button
                          onClick={() => {
                            setRenamingId(s.id);
                          ...
                          className="w-full px-3 py-1.5 text-xs text-left flex items-center gap-2 hover:bg-muted"
                        >
                          <Pencil className="h-3.5 w-3.5" /> নাম পরিবর্তন
                        </button>
                        <button
                          onClick={() => deleteSession(s.id)}
                          className="w-full px-3 py-1.5 text-xs text-left flex items-center gap-2 hover:bg-muted text-red-600"
                        >
  ...
  return (
    <div className="min-h-[100dvh] flex bg-muted">
      {/* Desktop sidebar */}
      ...
      <main className="flex-1 min-w-0 flex flex-col w-full">
        <header className="px-3 sm:px-4 py-2.5 sm:py-3 bg-card border-b flex items-center gap-2 sticky top-0 z-10">
          <button
          ...
          <button
            ...
            className="hidden md:flex h-9 w-9 rounded-full hover:bg-muted items-center justify-center"
            aria-label="ফিরে যান"
          >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] sm:text-[11px] text-muted-foreground truncate">
              আপনার AI কৃষি সহকারী
            </p>
          </div>
          {messages.some((m) => m.role === "user") &&
            (savedBadge ? (
              <span className="inline-flex items-center gap-1 h-9 px-3 rounded-full bg-muted text-muted-foreground text-xs font-semibold">
              ...
              <button
                onClick={saveChat}
                disabled={saving}
                className="inline-flex items-center gap-1.5 h-9 px-3 sm:px-4 rounded-full bg-primary text-primary-foreground text-xs sm:text-sm font-semibold hover:bg-primary/90 active:scale-95 transition disabled:opacity-60 animate-pulse"
              >
                {saving ? (
        ...
        >
          {messages.map((m, i) => (
            ...
            >
              ...
              {m.role === "assistant" && (
                <div className="mt-1 ml-1 flex items-center gap-2 text-[11px] text-muted-foreground">
        ...
        <div
          className="fixed bottom-[calc(6rem+env(safe-area-inset-bottom))] md:bottom-0 left-0 right-0 md:left-auto md:right-0 bg-card border-t border-border z-20 md:ml-64"
        ...
        >
            ...
            <div className="px-3 pt-2 pb-1 flex gap-2 overflow-x-auto scrollbar-hide">
              {suggestions.map((s, i) => (
                <button
                  key={i}
                  onClick={() => sendText(s)}
                  className="shrink-0 text-[11px] sm:text-xs px-3 py-1.5 rounded-full bg-primary/10 text-primary border border-primary/20 hover:bg-primary/20 active:scale-95 transition"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="p-2.5 sm:p-3 flex gap-2 items-center max-w-[900px] mx-auto">
            <input
              ...
              className="flex-1 min-w-0 h-11 px-4 rounded-full bg-muted text-sm outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-70"
            />
            <button
            ...
            <button
              ...
              className="h-11 w-11 shrink-0 rounded-full bg-primary text-primary-foreground flex items-center justify-center disabled:opacity-50 active:scale-95 transition"
              aria-label="পাঠান"
            >
      ...
      {warning && (
        ...
        >
          <div
            className="bg-card rounded-2xl p-5 max-w-sm w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="font-bold text-base mb-2">⚠️ চ্যাট সেভ হয়নি</h3>
            <p className="text-sm text-muted-foreground mb-4">
              এই কথোপকথন সেভ না করলে হারিয়ে যাবে।
            </p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => {
                  const a = warning.action;
                  setWarning(null);
                  a();
                }}
              ...
              <button
                ...
                className="h-10 px-4 rounded-lg text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center gap-1.5"
              >
