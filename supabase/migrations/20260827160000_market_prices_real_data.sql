-- Real-world market price quality and provenance improvements.
-- Run this migration once in Lovable Cloud SQL Editor.
...
-- Fill previous_price from the latest comparable community report when the
-- client does not provide one. This makes trend arrows and price alerts work
-- for normal manual submissions without trusting browser-side calculations.
...
-- Keep the prediction history source-aware while combining same-day records.
...
-- Refresh the official DAM report every day at 08:15 Asia/Dhaka.
-- This follows the same authenticated hook pattern used by the existing
-- crop-reminder scheduler in this project.
...
SELECT cron.schedule(
  ...
  SELECT net.http_post(
    ...
    headers := '{"Content-Type":"application/json","apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml5bXdibGZsdGN4YXFqbHB3eW90Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzk2OTg5NjUsImV4cCI6MjA5NTI3NDk2NX0.10teMxa8N22OzCUewExju68fJZAZpt_lGgDeVhsHzQk"}'::jsonb,
    body := '{}'::jsonb
  );
  $$
);
